// From: https://github.com/KroneCorylus/.dotfiles/blob/main/ghostty/.config/ghostty/shaders/dither.glsl
...
#define PATTERN_SCALE 1.2

float getBayerFromPacked(int x, int y) {
	y = int(y / PATTERN_SCALE);
	x = int(x / PATTERN_SCALE);
    int idx = (x & 3) + ((y & 3) << 2);
